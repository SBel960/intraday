...
1 pour une ``QlabError``, 2 pour un bug. Chaque échec est écrit sur stderr et, si un journal
est fourni, dans le journal JSONL.
"""
...
def run_cli(entry: Callable[[], int], *, journal: JsonLog | None = None) -> int:
    ...
    ``entry`` renvoie son propre code en cas de succès. Une ``QlabError`` donne 1 avec un
    message d'une ligne ; toute autre exception donne 2 avec la trace complète. Les
    ``BaseException`` hors ``Exception`` (Ctrl-C, ``SystemExit``) ne sont pas interceptées.
    """
    try:
        return entry()
